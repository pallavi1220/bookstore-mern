    ...
    <nav className="  z-50 relative flex bg-zinc-800 text-white px-8 py-2  items-center justify-between">
        <Link to="/" className="flex items-center">
            <img className="h-10 me-4"
             ...
             <h1 className="text-2xl font-semibold">Bookiverse</h1>
        </Link>
        <div className="nav-links-bookheaven block md:flex items-center gap-4 ">
           ...
           ? (<Link to={items.link}
             ...
             key={i}>{items.title}</Link>) : (<Link to={items.link}
            ...
            <button className=" block md:hidden text-white text-2xl hover:text-zinc-400"
             onClick={()=>MobileNav === "hidden"? setMobileNav("block"):setMobileNav("hidden")}><FaGripLines /></button>
        </div>
